...
import { Bluescoin } from "../target/types/bluescoin";
import { BluescryptoStaking } from "../target/types/bluescrypto_staking";

describe("bluescrypto-staking", () => {
    
    // Configure the client to use the local cluster.
    anchor.setProvider(anchor.AnchorProvider.env());
    
    const token_program = anchor.workspace.Bluescoin as Program<Bluescoin>
    const staking_program = anchor.workspace.BluescryptoStaking as Program<BluescryptoStaking>
    ...
    it("Mint token", async () => {
        ...
        await staking_program.methods.stake(1).accounts({
            from: fromTokenAccount.address,
            authority: key,
            tokenProgram: TOKEN_PROGRAM_ID,
            stakingStorage,
